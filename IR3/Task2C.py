...
def write_unigram_contents (corpus_map, file_location, filename):
...
def write_bigram_contents (corpus_map, file_location, filename):
...
# Method to write all unigrams to a file per document along with the unigram frequency
def write_trigram_contents (corpus_map, file_location, filename):
    # Format written to the file(Term, documentID, term frequency in the document)
    folder = os.path.join (file_location, filename)
    file_key = filename.split (".")[0]
...
def build_unigrams (tokens, filename):
    ...
    write_unigram_contents (corpus_map, Unigram_Location, filename)
...
def build_bigrams (tokens, filename):
    ...
    write_bigram_contents (corpus_map, Bigram_Location, filename)
...
def build_trigrams (tokens, filename):
    ...
    write_trigram_contents (corpus_map, Trigram_Location, filename)
